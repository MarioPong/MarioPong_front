// console.log('게임 모드:', window.gameMode); // 'one' 또는 'two'
...
window.initPongGame = function (gameMode) {
  console.log('게임 모드:', gameMode);

  window.gameMode = gameMode; // 글로벌로 저장
...
`;
  ...
  function drawMiddleLine() {
    ctx.beginPath();
    ctx.setLineDash([10, 15]); // 점선 패턴
    ctx.moveTo(canvas.width / 2, 0);
    ctx.lineTo(canvas.width / 2, canvas.height);
    ctx.strokeStyle = '#000';
    ctx.stroke();
    ctx.setLineDash([]); // 패턴 리셋
  }
